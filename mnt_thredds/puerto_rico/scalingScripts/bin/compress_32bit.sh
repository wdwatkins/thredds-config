...
if [ $scale != 1 ];  then
    ...
    ncap2  -O -L 0 --cnk_plc=unchunk --fl_fmt=netcdf4 -s "${var}=long(${scale}*${var})" ${SRC_DIR}/${file} ${PROC_DIR}/${file}
    ncatted -h -O -a scale_factor,${var},c,f,${scale_factor} ${PROC_DIR}/${file}
else
    ncap2 -h -O -L 0 --cnk_plc=unchunk --fl_fmt=netcdf4 -s "${var}=long(${var})" ${SRC_DIR}/${file} ${PROC_DIR}/${file}
fi
