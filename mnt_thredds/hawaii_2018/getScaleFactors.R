...
options(scipen=20)
library(dplyr)
scales <- read.table('/Volumes/RAID0/notaro/USGS/notaro_ranges.txt', sep = " ",
                     stringsAsFactors = FALSE, col.names = c("fileName", "varInFile", "min", "max"))
...
write.table(x = computedFactors_16, file = "notaro_scales.csv", row.names = FALSE,
          quote = TRUE, sep = "\t")
write.table(x = bigFactors, file = "notaro_scales_32.csv", row.names = FALSE,
            quote = TRUE, sep = "\t")
